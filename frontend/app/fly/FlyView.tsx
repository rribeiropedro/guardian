...
interface State {
  ...
  velLng: number; velLat: number; velAlt: number
...
export default function FlyView({ initialLat, initialLng, locationName }: FlyViewProps) {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      s.pitch = Math.max(PITCH_MIN, Math.min(PITCH_MAX, s.pitch + e.movementY * SENSITIVITY * (180 / Math.PI)))
    }
  ...
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      ...
      if ([' ', 'w', 'a', 's', 'd'].includes(e.key.toLowerCase()) && stateRef.current.active) e.preventDefault()
    }
    const onUp = (e: KeyboardEvent) => stateRef.current.keys.delete(e.key.toLowerCase())
